...
function renderTorres(){
 ...
 app.innerHTML=`<div class="tv-shell tv-torres-screen"><div class="tv-head"><div class="tv-brand"><img src="logo-viva.png"><div><h1>TV TORRES • VISÃO DA EMPRESA</h1><p>Viva Conecta Telecom • performance geral</p></div></div><div class="tv-meta"><span class="live-bullet"></span>ATUALIZAÇÃO AUTOMÁTICA • ${data.company.month}</div></div>
...
function renderEquipe(){
 ...
 app.innerHTML=`<div class="tv-shell tv-equipe-screen"><div class="tv-head"><div class="tv-brand"><img src="logo-viva.png"><div><h1>TV EQUIPE • PERFORMANCE INDIVIDUAL</h1><p>Resultado individual + pódio automático</p></div></div><div class="tv-meta"><span class="live-bullet"></span>ATUALIZAÇÃO AUTOMÁTICA • ${data.company.month}</div></div>
...
function exportIndividualExcel(){
 ...
 const sheets=[
  {name:'Resumo',headers:['Indicador','Valor'],widths:[24,24],rows:[['Colaborador',d.person.name],['Equipe',d.person.team||''],['Período',d.label],['Meta',d.meta],['Realizado',d.realized],['Atingimento %',d.ating],['Diferença',diff],['Posição',d.pos||0],['Lançamentos',d.hist.length]]},
...
function applyProduction(h,sign=1){const pi=data.people.findIndex(p=>p.name===h.personName),pr=data.products.findIndex(p=>p.name===h.productName);if(pi>=0)data.people[pi].realized=Number(data.people[pi].realized||0)+sign*Number(h.revenue||0);if(pr>=0)data.products[pr].realized=Number(data.products[pr].realized||0)+sign*Number(h.qty||0);data.company.realized=Number(data.company.realized||0)+sign*Number(h.revenue||0);const team=h.teamName||(pi>=0?data.people[pi].team:'');const ti=data.teams.findIndex(t=>t.name===team);if(ti>=0)data.teams[ti].realized=Number(data.teams[ti].realized||0)+sign*Number(h.revenue||0)}
...
async function removeProduct(i){
 const p=data.products[i];if(!p)return;
 const hasHistory=(data.productionHistory||[]).some(h=>h.productName===p.name);
